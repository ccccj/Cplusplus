...
size_t String::Find(const char* str) {
	for (size_t i = 0; i < _size; ++i) {
		size_t j = 0, k = i;
		while (str[j] != '\0' && _str[k] != '\0' && str[j++] == _str[k++]) {
			;
		}
		if (str[j] == '\0') {
			return i;
...
void Test_Find_str() {
	TEST_HEADER;
	String s("abceddfabcdgrmeo");
	size_t ret = s.Find("abcd");
	printf("ret expert: 0 actual: %lu\n", ret);
	ret = s.Find("fab");
...
void Test_operator() {
	TEST_HEADER;
	String s("hello");
	printf("s[1] expert: e actual: %c\n", s[1]);
	printf("s expert: hello actual: %s\n", s.c_str());
	s[3] = 'a';
	printf("s expert: helao actual: %s\n", s.c_str());
}
